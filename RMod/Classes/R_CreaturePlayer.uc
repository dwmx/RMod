...
//  R_CreaturePlayer
//==============================================================================
class R_CreaturePlayer extends R_RunePlayer config(RMod);
...
{
    // This will have no effect on creatures
}
...
function ApplyOwnerAndProxySkelGroupFlags()
{
    ...
    Log("ApplyOwnerAndProxySkelGroupFlags");
...
/**
*   PlayFiring (override)
*   Overridden to perform shield bash when defending
*/
function PlayFiring()
{
    ...
    if(AnimProxy != None)
    {
        if(AnimProxy.GetStateName() == 'Defending')
        {
            if(Physics == PHYS_Walking && GetStateName() == 'PlayerWalking')
            {
                DoShieldBash();
            }
        }
...
function DoShieldBash()
{
    ...
    PlayUninterruptedAnimModified('shieldbash', false);
}
...
function PlayUninterruptedAnimModified(Name AnimToPlay, optional bool bKillVelocity)
{
    if(bKillVelocity)
    {
        Velocity.X = 0;
