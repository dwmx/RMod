...
var() enum EBumpType
{
	...
	BT_CreatureBump,	// RUNE:  Non-player pawn bump
...
//-----------------------------------------------------------------------------
// Movement parameters.
var() float      MoveTime[16];      // Time to spend moving between keyframes. RUNE:  Variable movetimes
...
var() name		 InterpolateEvent[16]; // RUME:  Events to call at each interpolation point
...
function InterpolateEnd( actor Other )
{
	...
	{
		// Chain to previous.
		InterpolateTo(KeyNum-1, MoveTime[KeyNum-1]); // RUNE:  Added variable Movetimes
	}
	else if( KeyNum<NumKeys-1 && KeyNum>OldKeyNum )
	{
		// Chain to next.
		InterpolateTo(KeyNum+1, MoveTime[KeyNum]); // RUNE:  Added variable Movetimes
	}
	else
	{
...
function DoOpen()
{
	bOpening = true;
	bDelaying = false;
	InterpolateTo( 1, MoveTime[0] ); // RUNE:  Added variable Movetimes
...
function DoClose()
{
	...
	bOpening = false;
	bDelaying = false;
	InterpolateTo( Max(0,KeyNum-1), MoveTime[Max(0, KeyNum-1)]); // RUNE:  Added variable Movetimes
...
function Bump( actor Other )
{
	...
	if ( (BumpType == BT_PawnBump) && (Other.Mass < 10) )
		return;
	if((BumpType == BT_CreatureBump) && (Other.Mass < 10 || P.bIsPlayer))
...
state() BumpOpenTimed
{
	...
	function Bump( actor Other )
	{
		...
		if ( (BumpType == BT_PawnBump) && (Other.Mass < 10) )
			return;
		if((BumpType == BT_CreatureBump) && (Other.Mass < 10 || Pawn(Other).bIsPlayer))
			return; // RUNE:  Non-player pawn bump
