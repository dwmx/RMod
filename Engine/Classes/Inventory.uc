...
var		bool		bExpireWhenTossed;	// Expire after ExpireTime when tossed out
...
var() int         PlayerViewMesh;     // Mesh to render (within skelmodel).
...
var() int         PickupViewMesh;     // Mesh to render (within skelmodel).
var() float       PickupViewScale;    // Mesh scale.

// 3rd person mesh.
var() int         ThirdPersonMesh;    // Mesh to render (within skelmodel).
var() float       ThirdPersonScale;   // Mesh scale.

//-----------------------------------------------------------------------------
// Status bar info.

var() texture     StatusIcon;         // Icon used with charge/power count.
...
var(Sounds) sound PickupSound, ActivateSound, DeActivateSound, RespawnSound;
var(Sounds) sound DropSound;
...
//
function ReduceDamage( out int BluntDamage, out int SeverDamage, name DamageType, vector HitLocation )
{
	local Inventory FirstArmor;
	local int ReducedAmount,ArmorDamage;
	
	if( BluntDamage<0 && SeverDamage<0)
	{
		BluntDamage = 0;
		SeverDamage = 0;
		return;
	...
	ReducedAmount = BluntDamage;
	FirstArmor = PrioritizeArmor(BluntDamage, DamageType, HitLocation);
...
// ===== INVENTORY SPECIALS =====
// These are functions that are called on the inventory item from an actor
// when that actor received a particular notify function
...
auto state Pickup
{
	...
	function Touch( actor Other )
	{
		...
		// Logic for bAutoPickup is in PlayerProxy CanPickup()
		...
		if( ValidTouch(Other) )
		{
			...
			Pawn(Other).AcquireInventory(Copy); // RUNE
		}
	}
...
defaultproperties
{
     bExpireWhenTossed=True
     bRotatingPickup=True
     PickupMessage="None"
     ItemArticle="a"
     PlayerViewScale=1.000000
     BobDamping=0.960000
     ExpireTime=45.000000
