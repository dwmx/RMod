...
//=============================================================================
class R_SavedMove extends Info;

var R_SavedMove NextMove;   // Next move in linked list.
var float TimeStamp;        // Time of this move.
var float Delta;            // Distance moved.
var int MergeCount;         // Additional moves merged into this.
var bool bRun;
var bool bDuck;
...
final function bool CanMergeAccel(vector OtherAccel)
{
    ...
    if (VSize(RealAccel - OtherAccel) > 1 && Normal(RealAccel) dot Normal(OtherAccel) < 0.95)
    {
        return false;
    }
    return true;
...
{
    if (bPressedJump || (DodgeMove >= DODGE_Left && DodgeMove <= DODGE_Back) || !CanMergeAccel(NewAcceleration))
        return true;
...
{
    if (bForceFire || bForceAltFire || bPressedJump || (DodgeMove >= DODGE_Left && DodgeMove <= DODGE_Back) || MergeCount >= 31 || !CanMergeAccel(OldAcceleration))
        return false;

    return true;
}

function byte CompressFlags()
{
...
function int CompressOld()
{
    local int Compressed;
    local vector BuildAccel;

    BuildAccel = 0.05 * Acceleration + vect(0.5, 0.5, 0.5);
    Compressed = (CompressAccel(BuildAccel.X) << 23) + (CompressAccel(BuildAccel.Y) << 15) + (CompressAccel(BuildAccel.Z) << 7);
