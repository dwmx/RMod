...
//==============================================================================
class R_DBStringManager extends Object;

const Utilities = Class'RBase.R_AUtilityLibrary';
const DebugLib = Class'RDebugTools.R_DBLibrary';

const LogCategory = 'Debug';
const LogSubCategory = 'StringManager';
...
function AddColor(Name Category, String Label, Color Color)
{
	...
	{
		Utilities.Static.RLog("AddColor failed -- GetCategoryIndex failed when it should not have", LogCategory, LogSubCategory);
		return;
	}

	if(ColorLegendArray[CategoryIndex].NumLabeledColors >= MAX_LABELED_COLORS)
	{
		Utilities.Static.RLog("AddColor failed -- too many colors, max is" @ MAX_LABELED_COLORS, LogCategory, LogSubCategory);
		return;
	}
...
function AddString(Name Category, String DebugString, optional String Label, optional Name StringType, optional int MetaData)
{
	...
	{
		Utilities.Static.RLog("StringManager debug string array full, cannot add string" @ DebugString, LogCategory, LogSubCategory);
		return;
	}
