class R_ATest_List_RemoveMultiple extends R_ATest_List abstract;

static function String GetTestNameString()
{
	return "Remove multiple objects from list";
}
...
static function bool RunTest(out String FailedResonString)
{
	...
	if(ListLength != 0)
	{
		FailedResonString = "List length after removing three mocks wrong -- expected 0, got" @ ListLength;
		return false;
	}
	if(ListNode.GetAtIndex(0) != None
	|| ListNode.GetAtIndex(1) != None
	|| ListNode.GetAtIndex(2) != None)
	{
		FailedResonString = "GetAtIndex should have returned None, but didn't";
		return false;
	}
